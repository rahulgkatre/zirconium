...
pub fn Tensor(comptime Array: type) type {
    return struct {
        ...
        idx_transformer: fn ([]const usize, []usize) void = struct {
            fn transform(_: []const usize, _: []usize) void {
                return;
            }
        }.transform,
        ...
        pub fn split(b: *const Self, comptime dim: u8, comptime count: u8) Split(dim, count) {
            ...
            const old_transformer = b.idx_transformer;
            const new_transformer = struct {
                fn transform(src: []const usize, dst: []usize) void {
                    std.debug.assert(src.len == t_ndims + 1);
                    @call(.auto, old_transformer, .{ src, dst });
                    dst[dim] = src[dim] * count + src[dim + 1];
                }
            }.transform;

            return .{
                .layout = layout,
                .idx_transformer = new_transformer,
            };
        }
...
test "split" {
    const t = Tensor([16][8]f32).init();
    const st = t.split(0, 4);

    try std.testing.expect(@TypeOf(st) == Tensor([4][4][8]f32));
    try std.testing.expectEqualSlices(usize, &.{ 32, 8, 1 }, st.layout.strides);
}
...
test "vectorize_nest_eval" {
    ...
    const vt = t.vectorize();
    const VT = @TypeOf(vt);
    try std.testing.expect(VT.Base == @Vector(8, i32));
    const nest = try vt.nest(arena.allocator());
    const Args = struct {
        b: VT.Buffer,
    };

    var vb = try vt.alloc(arena.allocator());
    ...
    const logic: loop.Logic(Args, Args) = comptime struct {
        ...
        // thread / group ids
        fn logic(b1: *const VT.Buffer, b2: *VT.Buffer, idx: []const usize) void {
            const val = b1.load(idx) + b1.constant(1);
            b2.store(val, idx);
        }
    }.logic;
    ...
    try std.testing.expectEqualSlices(i32, &(.{1} ** 128), vb.raw[0..128]);
}
